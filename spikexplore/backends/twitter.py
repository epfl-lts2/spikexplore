...
class TweetsGetterV1:
    ...
    def _filter_old_tweets(self, tweets):
        ...
        tweets_filt = filter(lambda t: datetime.strptime(t["created_at"], "%a %b %d %H:%M:%S +0000 %Y") >= days_limit, tweets)
    ...
    def get_user_tweets(self, username):
        ...
        # Test if ok
        try:
            user_tweets_raw = self.twitter_handle.get_user_timeline(
                screen_name=username, count=count, include_rts=True, tweet_mode="extended", exclude_replies=False
            )
            # remove old tweets
...
class TweetsGetterV2:
    def __init__(self, credentials, config):
        self.twitter_handle = TwitterAPI(credentials.consumer_key, credentials.consumer_secret, api_version="2", auth_type="oAuth2")
        self.config = config
    ...
    def _get_user_tweets(self, username, num_tweets, next_token):
        ...
        for k in user_tweets.keys():
            ...
            if "referenced_tweets" in user_tweets[k]:
                ref = list(filter(lambda x: x["type"] == "quoted" or x["type"] == "retweeted", user_tweets[k]["referenced_tweets"]))
                if ref:
                    ref_type = ref[0]["type"]
                    ref_txt = ""
            ...
            map(
                lambda x: (
                    x[0],
                    {
                        ...
                        "mentions": list(map(lambda y: y["username"], x[1].get("entities", {}).get("mentions", {}))),
                        "hashtags": list(map(lambda y: y["tag"], x[1].get("entities", {}).get("hashtags", {}))),
...
class TwitterNetwork:
    ...
    def get_edges(self, tweets_meta):
        ...
        if tmp.empty:
            return tmp
        edge_df = pd.DataFrame(tmp.tolist(), index=tmp.index).rename(columns={0: "tweet_id", 1: "weight"})
        return edge_df
    ...
    def get_nodes_properties(self, tweets_meta, tweets_dic):
        ...
        # global properties
        meta_df = pd.DataFrame.from_dict(tweets_meta, orient="index").sort_values("retweet_count", ascending=False)
        # hashtags statistics
    ...
    def add_graph_attributes(self, g, nodes_df, edges_df, nodes_info):
        ...
        g = add_node_attributes(g, self.tweets_getter.reshape_node_data(nodes_df), attr_dic=nodes_info.user_hashtags, attr_name="all_hashtags")
        return g
